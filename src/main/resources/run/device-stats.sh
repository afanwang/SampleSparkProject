...
CONFIG_FILE="$SCRIPT_DIR/device-events.conf"
...
## JAR list building
CLASSPATH_JAR=""
for entry in "$SCRIPT_DIR/../jars"/*.jar
do
  CLASSPATH_JAR="$CLASSPATH_JAR,$entry"   
  echo "$entry"
done

CLASSPATH_JAR=${CLASSPATH_JAR:(1)}
...
cmd="$SPARK_PATH/bin/spark-submit --master $SPARK_MASTER_URL --driver-memory 4g \
--jars $CLASSPATH_JAR \
--class $MAIN_CLASS \
$SCRIPT_DIR/$APPLICATION_JAR \
